...
import javabeans.BeanInfo;
import javabeans.IntrospectionException;
import javabeans.Introspector;
import javabeans.PropertyDescriptor;
...
public class BeanProxy extends AbstractProxy
{
    ...
    public Object getValue(Object instance, String propertyName)
    {
        ...
        if (!ignorePropertyErrors(context))
        {
            ...
            ex.setMessage("UNKNOWN_PROPERTY_ERROR");
...
/*                if (Log.isWarn() && logPropertyErrors(context))
                ...
                if (!ignorePropertyErrors(context))
                {
                    ...
                    MessageException ex = new MessageException();
                    ex.setMessage("FAILED_PROPERTY_READ_ERROR");
        ...
        else
        {
            ...
            if (!ignorePropertyErrors(context))
            {
                ...
                MessageException ex = new MessageException();
                ex.setMessage("NON_READABLE_PROPERTY_ERROR");
...
/*                    if (Log.isWarn() && logPropertyErrors(context))
                    ...
                    if (!ignorePropertyErrors(context))
                    {
                        // Failed to get property '{propertyName}' on type '{className}'.
                        MessageException ex = new MessageException();
                        ex.setMessage("FAILED_PROPERTY_WRITE_ERROR");
...
/*                if (Log.isWarn() && logPropertyErrors(context))
                ...
                if (!ignorePropertyErrors(context))
                {
                    ...
                    MessageException ex = new MessageException();
                    ex.setMessage("NON_WRITABLE_PROPERTY_ERROR");
