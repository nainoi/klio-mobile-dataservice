...
/*        Map<Class<?>, BeanInfo> map;
        synchronized (BEANINFO_CACHE) {
            ...
            if (map == null) {
                ...
                Contex.getAppContext().put(BEANINFO_CACHE, map);
            }
        }*/
...
/*        Map map = (Map) AppContext.getAppContext().get(BEANINFO_CACHE);
    ...
    private static synchronized BeanInfo findExplicitBeanInfo(Class beanClass) {
        ...
        try {
            return (javabeans.BeanInfo)instantiate(beanClass, name);
        } catch (Exception ex) {
            // Just drop through
        ...
        try {
            if (isSubclass(beanClass, javabeans.BeanInfo.class)) {
                return (javabeans.BeanInfo)beanClass.newInstance();
            }
        } catch (Exception ex) {
            // Just drop through
        }
        ...
        for (int i = 0; i < searchPath.length; i++) {
            ...
            if (!DEFAULT_INFO_PATH.equals(searchPath[i]) ||
                ...
                try {
                    ...
                    javabeans.BeanInfo bi = (javabeans.BeanInfo)instantiate(beanClass, fullName);
